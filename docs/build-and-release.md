...
| Build Windows x64 | Windows | `hookbuddy-windows-x64` | `.exe`、`latest.yml`、`.blockmap` |
| Build Linux x64 | Linux | `hookbuddy-linux-x64` | `.AppImage`、`.deb`、`latest-linux.yml` 等 |

Artifact 保留 14 天，可在对应 Workflow Run 页面下载。CI 使用 `--publish never`，electron-builder 不会自动上传到更新服务器。
...
- Release 说明由 `generate_release_notes: true` 自动生成。

## 自动更新

应用内自动更新基于 electron-updater，更新源为本仓库的 GitHub Releases（`electron-builder.yml` 的 `publish: provider: github`）。**系统通知已关闭**，以侧栏左下角 / 标题栏的向上箭头入口为主。

### 共同行为

- 生产环境启动时检查更新，运行期间每 6 小时复查；开发模式（`pnpm dev`）不检查。
- 仅接收正式稳定版 Release（忽略 draft / prerelease）。
- 更新状态由 Main 维护，经 Preload 白名单 IPC 推送到 Renderer；`appId` 固定为 **`com.huyuan.hybuddy`**。
- 检查/下载连续失败时 toast 提示并提供「重试」；首次失败静默重试一次。

### Windows（完整自动更新）

- 发现新版本后后台静默下载；下载完成后侧栏「设置」旁显示向上箭头。
- 点击箭头确认「立即重启并更新」或「稍后」；立即路径安装后自动重启；稍后 + 正常退出时自动安装。
- 存在运行中任务时阻止立即重启（R0 任务状态为 stub，接口已预留）。

### macOS（降级：下载 dmg 手动安装）

- 当前未配置 Apple 签名/公证（`identity: null`），**不走** electron-updater 静默安装管线。
- 检测到新版本即显示箭头；用户确认后 Main 从 GitHub Release 下载 `${name}-${version}.dmg` 到系统「下载」目录，完成后自动挂载打开。
- 用户需手动将应用拖入 Applications；未签名包首次打开可能需 Gatekeeper 放行（右键「打开」或系统设置允许）。
- 同版本 dmg 已存在且大小匹配时不重复下载，直接打开本地文件。
- 签名/公证就绪后可切回与 Windows 一致的静默自动更新（见 `specs/prds/prd-00002-auto-update.md` R1）。

### Release 资产要求

Release Assets 中的 `latest*.yml`、`.dmg` / `.exe` 与 `*.blockmap` 是更新元数据与安装包：**不要删除这些 Assets**，否则已装客户端无法检测或完成更新。

本地开发调试更新流程用根目录 `dev-app-update.yml`。
...
6. CI 自动创建 GitHub Release 并挂上各平台安装包；在 Releases 页面确认产物，按需补充说明或分发。
...
| `publish` | GitHub Releases（`0x00pluto/huyuan-hookbuddy`），electron-updater 更新源；构建仍用 `--publish never`，上传由 CI 的 release job 完成 |

签名与公证上线前再按环境补齐（macOS 自动更新依赖签名，见上文）。
